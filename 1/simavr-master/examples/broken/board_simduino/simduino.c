...
/*
 * called when the AVR change any of the pins on port B so lets update our buffer
 */
void
...
void
avr_special_init (avr_t * avr, void *data)
{
...
main (int argc, char *argv[])
{
  ...
  // this trick creates a file that contains /and keep/ the flash in the same state as it was
  // before. This allow the bootloader app to be kept, and re-run if the bootloader doesn't get a
  // new one
  {
    char path[1024];
    uint32_t base, size;
