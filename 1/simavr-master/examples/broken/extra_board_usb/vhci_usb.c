...
static void
vhci_usb_attach_hook (struct avr_irq_t *irq, uint32_t value, void *param)
{
...
char *setuprequests[] = { "GET_STATUS", "CLEAR_FEAT", "", "SET_FEAT", "", "SET_ADDR", "GET_DESCR",
			  "SET_DESCR", "GET_CONF", "SET_CONF"
};

static int
control_read (struct vhci_usb_t *p,
              struct _ep *ep,
              uint8_t reqtype,
              uint8_t req, uint16_t wValue, uint16_t wIndex, uint16_t wLength, uint8_t * data)
{
  assert (reqtype & 0x80);
  int ret;
  struct usbsetup buf = { reqtype, req, wValue, wIndex, wLength };
  struct avr_io_usb pkt = { ep->epnum, sizeof (struct usbsetup), (uint8_t *) & buf };
...
static int
control_write (struct vhci_usb_t *p,
               struct _ep *ep,
               uint8_t reqtype,
               uint8_t req, uint16_t wValue, uint16_t wIndex, uint16_t wLength, uint8_t * data)
{
...
static void
handle_status_change (struct vhci_usb_t *p,
                      struct usb_vhci_port_stat *prev, struct usb_vhci_port_stat *curr)
{
  ...
  if (curr->change & USB_VHCI_PORT_STAT_C_RESET
      && ~curr->status & USB_VHCI_PORT_STAT_RESET && curr->status & USB_VHCI_PORT_STAT_ENABLE)
    {
      //         printf("END OF RESET\n");
    }
  if (~prev->status & USB_VHCI_PORT_STAT_RESET && curr->status & USB_VHCI_PORT_STAT_RESET)
    {
  ...
  if (~prev->status & USB_VHCI_PORT_STAT_SUSPEND && curr->status & USB_VHCI_PORT_STAT_SUSPEND)
    printf ("port suspedning\n");
  if (prev->status & USB_VHCI_PORT_STAT_ENABLE && ~curr->status & USB_VHCI_PORT_STAT_ENABLE)
...
static int
get_ep0_size (struct vhci_usb_t *p)
{
  struct _ep ep0 = { 0, 8 };
  uint8_t data[8];
...
static void
handle_ep0_control (struct vhci_usb_t *p, struct _ep *ep0, struct usb_vhci_urb *urb)
{
  int res;
  if (urb->bmRequestType & 0x80)
    {
      res = control_read (p, ep0,
                          urb->bmRequestType,
                          urb->bRequest, urb->wValue, urb->wIndex, urb->wLength, urb->buffer);
      if (res >= 0)
        {
      ...
      switch (wrk.type)
        ...
        case USB_VHCI_WORK_TYPE_PROCESS_URB:
          ...
          if (wrk.work.urb.packet_count)
            wrk.work.urb.iso_packets = malloc (wrk.work.urb.packet_count
          ...
          if (usb_vhci_is_control (wrk.work.urb.type) && !(wrk.work.urb.epadr & 0x7f))
            ...
            {
              struct avr_io_usb pkt = { wrk.work.urb.epadr, wrk.work.urb.buffer_actual,
					wrk.work.urb.buffer
              };
...
void
vhci_usb_init (struct avr_t *avr, struct vhci_usb_t *p)
{
  p->avr = avr;
  pthread_t thread;
...
void
vhci_usb_connect (struct vhci_usb_t *p, char uart)
{
