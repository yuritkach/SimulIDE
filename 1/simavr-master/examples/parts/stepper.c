...
stepper_update_timer (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
...
static void
stepper_dir_hook (struct avr_irq_t *irq, uint32_t value, void *param)
{
  stepper_p p = (stepper_p) param;
...
static void
stepper_enable_hook (struct avr_irq_t *irq, uint32_t value, void *param)
{
  stepper_p p = (stepper_p) param;
  p->enable = ! !value;
  printf ("%s (%s) %d pos %.4f\n", __func__, p->name,
...
static void
stepper_step_hook (struct avr_irq_t *irq, uint32_t value, void *param)
{
  stepper_p p = (stepper_p) param;
  if (!p->enable)
    return;
...
void
stepper_init (struct avr_t *avr,
              stepper_p p,
              char *name,
              float steps_per_mm, float start_position, float max_position, float endstop_position)
{
  p->avr = avr;
  strcpy (p->name, name);
...
void
stepper_connect (stepper_p p,
                 avr_irq_t * step,
                 avr_irq_t * dir, avr_irq_t * enable, avr_irq_t * endstop, uint16_t flags)
{
...
float
stepper_get_position_mm (stepper_p p)
{
