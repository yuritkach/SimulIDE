...
#define HZ (50)
#define PERIOD_USECS (USECS_PER_SECOND / HZ)

static avr_cycle_count_t
switch_auto (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
  ac_input_t *b = (ac_input_t *) param;
  b->value = !b->value;
  avr_raise_irq (b->irq + IRQ_AC_OUT, b->value);
  // Setup recall time
  return when + avr_usec_to_cycles (avr, PERIOD_USECS);
}

static const char *name = ">ac_input";

void
ac_input_init (avr_t * avr, ac_input_t * b)
{
  b->irq = avr_alloc_irq (&avr->irq_pool, 0, IRQ_AC_COUNT, &name);
  b->avr = avr;
  b->value = 0;
  avr_cycle_timer_register_usec (avr, PERIOD_USECS, switch_auto, b);
  printf ("ac_input_init period %duS or %d cycles\n",
          PERIOD_USECS,
          (int) avr_usec_to_cycles (avr, PERIOD_USECS));
}
