  ...
  while (1)
    {
      ...
      if (FD_ISSET (p->s, &read_set))
        {
          ...
          ssize_t r =
            recvfrom (p->s, buffer, sizeof (buffer) - 1, 0, (struct sockaddr *) &p->peer, &len);
	  ...
	  // write them in fifo
          ...
          /*size_t r = */ sendto (p->s, buffer, len, 0, (struct sockaddr *) &p->peer,
                                  sizeof (p->peer));
...
void
uart_udp_init (struct avr_t *avr, uart_udp_t * p)
{
  p->avr = avr;
...
void
uart_udp_connect (uart_udp_t * p, char uart)
{
