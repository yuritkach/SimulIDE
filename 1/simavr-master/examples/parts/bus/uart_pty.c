 ...
 */
static void
uart_pty_in_hook (struct avr_irq_t *irq, uint32_t value, void *param)
{
  ...
  while (1)
    {
      ...
      /* DO NOT call this, this create a concurency issue with the FIFO that can't be solved cleanly
       * with a memory barrier uart_pty_flush_incoming(p);
       */
    }
  return NULL;
}
  ...
  for (int ti = 0; ti < 1 + hastap; ti++)
    {
      ...
      if (openpty (&m, &s, p->port[ti].slavename, NULL, NULL) < 0)
        {
          fprintf (stderr, "%s: Can't create pty: %s", __FUNCTION__,
                   strerror (errno));
          return;
        }
      struct termios tio;
    ...
    if (p->port[ti].s)
      {
        ...
        if (symlink (p->port[ti].slavename, link) != 0)
          {
            fprintf (stderr, "WARN %s: Can't create %s: %s", __func__, link,
                     strerror (errno));
          }
        else
          {
