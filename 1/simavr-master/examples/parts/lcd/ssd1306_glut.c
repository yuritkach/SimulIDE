  ...
  if (invert)
    {
      glColor4f (ssd1306_colours[oled_colour_g][0],
                 ssd1306_colours[oled_colour_g][1], ssd1306_colours[oled_colour_g][2], opacity);
    }
  else
    {
...
{
  uint8_t seg_remap_default = ssd1306_get_flag (part, SSD1306_FLAG_SEGMENT_REMAP_0);
  uint8_t seg_comscan_default = ssd1306_get_flag (part, SSD1306_FLAG_COM_SCAN_NORMAL);
  ...
  if (seg_remap_default && seg_comscan_default)
  ...
  else if (seg_remap_default && !seg_comscan_default)
    {
      ...
      return ssd1306_gl_reverse_byte (part->vram[part->pages - 1 - page][column]);
    }

  else if (!seg_remap_default && !seg_comscan_default)
    {
      // Upside down display
      return ssd1306_gl_reverse_byte (part->
                                      vram[part->pages - 1 - page][part->columns - 1 - column]);
    }
  else if (!seg_remap_default && seg_comscan_default)
    {
  ...
  for (int p = 0; p < part->pages; p++)
    {
      ...
      glPopMatrix ();
      // Next page
      glTranslatef (0, (part->rows / part->pages) * pix_size_g + pix_gap_g, 0);
    }
}
