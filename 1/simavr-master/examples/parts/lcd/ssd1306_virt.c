...
ssd1306_update_command_register (ssd1306_t * part)
{
  switch (part->spi_data)
    ...
    case SSD1306_VIRT_SET_SEG_REMAP_127:
      ssd1306_set_flag (part, SSD1306_FLAG_SEGMENT_REMAP_0, 0);
    ...
    case SSD1306_VIRT_SET_COM_SCAN_INVERTED:
      ssd1306_set_flag (part, SSD1306_FLAG_COM_SCAN_NORMAL, 0);
...
static const char *irq_names[IRQ_SSD1306_COUNT] =
  {[IRQ_SSD1306_SPI_BYTE_IN] = "=ssd1306.SDIN",[IRQ_SSD1306_RESET] =
   "<ssd1306.RS",[IRQ_SSD1306_DATA_INSTRUCTION] = "<ssd1306.RW",[IRQ_SSD1306_ENABLE] = "<ssd1306.E",
...
{
  avr_connect_irq (avr_io_getirq (part->avr, AVR_IOCTL_SPI_GETIRQ (0),
                                  SPI_IRQ_OUTPUT), part->irq + IRQ_SSD1306_SPI_BYTE_IN);

  avr_connect_irq (avr_io_getirq (part->avr,
                                  AVR_IOCTL_IOPORT_GETIRQ (wiring->chip_select.port),
                                  wiring->chip_select.pin), part->irq + IRQ_SSD1306_ENABLE);

  avr_connect_irq (avr_io_getirq (part->avr,
                                  AVR_IOCTL_IOPORT_GETIRQ (wiring->data_instruction.port),
                                  wiring->data_instruction.pin),
                   part->irq + IRQ_SSD1306_DATA_INSTRUCTION);

  avr_connect_irq (avr_io_getirq (part->avr,
                                  AVR_IOCTL_IOPORT_GETIRQ (wiring->reset.port),
                                  wiring->reset.pin), part->irq + IRQ_SSD1306_RESET);
}
