...
hd44780_gl_init ()
{
  ...
  glTexImage2D (GL_TEXTURE_2D, 0, 4,
                lcd_font.width, lcd_font.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, lcd_font.pixel_data);
...
void
glputchar (char c, uint32_t character, uint32_t text, uint32_t shadow)
{
  int index = c;
...
void
hd44780_gl_draw (hd44780_t * b,
                 uint32_t background, uint32_t character, uint32_t text, uint32_t shadow)
{
  int rows = b->w;
  int lines = b->h;
  int border = 3;
