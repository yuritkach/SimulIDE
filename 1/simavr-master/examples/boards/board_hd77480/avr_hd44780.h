...
/*
 * Send byte b to the LCD.  rs is the RS signal (register select), 0 selects instruction register, 1
 * selects the data register.
 */
void hd44780_outbyte(uint8_t b, uint8_t rs);

/*
 * Read one byte from the LCD controller.  rs is the RS signal, 0 selects busy flag (bit 7) and
 * address counter, 1 selects the data register.
 */
...
/*
 * Select the entry mode.  inc determines whether the address counter auto-increments, shift selects
 * an automatic display shift.
 */
#define HD44780_ENTMODE(inc, shift) \
  (0x04 | ((inc)? 0x02: 0) | ((shift)? 1: 0))

/*
 * Selects disp[lay] on/off, cursor on/off, cursor blink[ing] on/off.
 */
...
/*
 * Function set.  if8bit selects an 8-bit data path, twoline arranges for a two-line display,
 * font5x10 selects the 5x10 dot font (5x8 dots if clear).
 */
