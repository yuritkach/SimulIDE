...
static void
special_deinit_longjmp_cb (struct avr_t *avr, void *data)
{
...
avr_t *
tests_init_avr (const char *elfname)
{
  tests_cycle_count = 0;
  map_stderr (); // WARNING: modify stdout and stderr
...
int
tests_run_test (avr_t * avr, unsigned long run_usec)
{
  if (!avr)
  ...
  avr_cycle_timer_register_usec (avr, run_usec, cycle_timer_longjmp_cb, &jmp);
...
int
tests_init_and_run_test (const char *elfname, unsigned long run_usec)
{
...
/* static void buf_output_cb(avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param) { */
static void
buf_output_cb (struct avr_irq_t *irq, uint32_t value, void *param)
{
  // Check buffer, reallocate if necessary and copy char
  struct output_buffer *buf = param;

  if (!buf)
    fail ("Internal error: buf == NULL in buf_output_cb()");
  if ((buf->alloclen == 0) ||
      (buf->currlen > buf->alloclen - 1))
    fail ("Internal error");
  ...
  // printf("%c", (char) value); // uncomment and restore stdout to echo AVR UART
}

void
tests_assert_uart_receive_avr (avr_t * avr, unsigned long run_usec, const char *expected, char uart)
{
...
void
tests_assert_uart_receive (const char *elfname, unsigned long run_usec, const char *expected, char uart)
{
  avr_t *avr = tests_init_avr (elfname);
...
void
tests_assert_cycles_at_least (unsigned long n)
{
...
void
tests_assert_cycles_at_most (unsigned long n)
{
  if (tests_cycle_count > n)
...
void
tests_assert_cycles_between (unsigned long min, unsigned long max)
{
