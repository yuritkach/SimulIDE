...
{       
  //
  // start the 16 bits timer, with default "normal" waveform and no interupt enabled. This just
  // increments TCNT1 at a regular rate
  //
  ...
  //
  // now enable a tick counter using an asynchronous mode
  //
  ASSR |= (1 << AS2);   // use "external" 32.7k crystal source
  // use CLK/8 prescale value, clear timer/counter on compareA match toggle OC2A pin too
