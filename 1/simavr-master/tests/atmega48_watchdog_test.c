...
int main()
{
  ...
  // NOTE: since the Change Enable bit is no longer on, it should not change the timer value that is
  // already set!
  WDTCSR = (1 << WDIE);
  ...
  // when arriving here, the watchdog timer interupt was called and woke up the core from sleep, so
  // we can just quit properly
  cli();
  sleep_cpu();
}
