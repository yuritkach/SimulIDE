...
/* TODO actually pay attention to endpoint memory allocation ? buggy endpoint configuration doesn't
   matter in the simulator now. */
...
const uint8_t num_endpoints = 5;   //sizeof (struct usb_internal_state.ep_state) / sizeof (struct
				   //usb_internal_state.ep_state[0]);

static uint8_t
current_ep_to_cpu (avr_usb_t * p)
{
  return p->io.avr->data[p->r_usbcon + uenum];
}

static struct _epstate *
get_epstate (avr_usb_t * p, uint8_t ep)
{
...
static void
raise_ep_interrupt (struct avr_t *avr, avr_usb_t * p, uint8_t ep, enum epints irq)
{
...
static void
raise_usb_interrupt (avr_usb_t * p, enum usbints irq)
{
...
static void
reset_endpoints (struct avr_t *avr, avr_usb_t * p)
{
  memset (&p->state->ep_state[1], 0, sizeof p->state->ep_state - sizeof p->state->ep_state[0]);
}

static int
ep_fifo_empty (struct _epstate *epstate)
{
  return epstate->bank[epstate->current_bank].tail == 0;
}

static int
ep_fifo_full (struct _epstate *epstate)
{
  return epstate->bank[epstate->current_bank].tail >= (8 << epstate->uecfg1x.epsize);
}

static uint8_t
ep_fifo_size (struct _epstate *epstate)
{
...
static uint8_t
ep_fifo_count (struct _epstate *epstate)
{
  return epstate->bank[epstate->current_bank].tail;
}

static int
ep_fifo_cpu_readbyte (struct _epstate *epstate)
{
  uint8_t i, j;
...
static int
ep_fifo_cpu_writebyte (struct _epstate *epstate, uint8_t v)
{
  if (!epstate->ueconx.epen)
    {
      printf ("WARNING! Adding bytes to non configured endpoint\n");
...
static int
ep_fifo_usb_read (struct _epstate *epstate, uint8_t * buf)
{
  if (!epstate->ueconx.epen)
    {
...
static int
ep_fifo_usb_write (struct _epstate *epstate, uint8_t * buf, uint8_t len)
{
  if (!epstate->ueconx.epen)
    {
      printf ("WARNING! Adding bytes to non configured endpoint\n");
...
static uint8_t
avr_usb_ep_read_bytecount (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
...
static void
avr_usb_udaddr_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  if (v & 0x80)
...
static void
avr_usb_uenum_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
...
static uint8_t
avr_usb_ep_read_ueintx (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_usb_t *p = (avr_usb_t *) param;
...
static void
avr_usb_ep_write_ueintx (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_usb_t *p = (avr_usb_t *) param;
  uint8_t ep = current_ep_to_cpu (p);
...
static uint8_t
avr_usb_ep_read (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_usb_t *p = (avr_usb_t *) param;
...
static void
avr_usb_ep_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_usb_t *p = (avr_usb_t *) param;
  struct _epstate *epstate = get_epstate (p, current_ep_to_cpu (p));
  uint8_t laddr = addr - p->r_usbcon;
  ...
  switch (laddr)
    ...
    case uecfg1x:
      ...
      avr_core_watch_write (avr, p->r_usbcon + uesta0x, epstate->uesta0x.v);
      break;
    case uesta0x:
...
static uint8_t
avr_usb_ep_read_data (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_usb_t *p = (avr_usb_t *) param;
...
static void
avr_usb_ep_write_data (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_usb_t *p = (avr_usb_t *) param;
...
static void
avr_usb_pll_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  v |= (v >> 1) & 1;
  avr_core_watch_write (avr, addr, v);
}
...
avr_cycle_count_t
sof_generator (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
  avr_usb_t *p = (avr_usb_t *) param;
...
static int
avr_usb_ioctl (struct avr_io_t *io, uint32_t ctl, void *io_param)
{
...
void
avr_usb_reset (struct avr_io_t *io)
{
  avr_usb_t *p = (avr_usb_t *) io;
  uint8_t i;
...
static void
avr_usb_dealloc (struct avr_io_t *port)
{
...
static void
register_io_ep_readwrite (avr_t * avr, avr_usb_t * p, uint8_t laddr)
{
...
static void
register_vectors (avr_t * avr, avr_usb_t * p)
{
...
void
avr_usb_init (avr_t * avr, avr_usb_t * p)
{
  p->io = _io;
