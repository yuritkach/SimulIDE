...
void
avr_global_logger_set (avr_logger_p logger)
{
...
void
avr_set_command_register (avr_t * avr, avr_io_addr_t addr)
{
  if (addr)
...
static void
_avr_io_console_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
...
void
avr_set_console_register (avr_t * avr, avr_io_addr_t addr)
{
  if (addr)
...
void
avr_loadcode (avr_t * avr, uint8_t * code, uint32_t size, avr_flashaddr_t address)
{
  if ((address + size) > avr->flashend + 1)
    {
      AVR_LOG (avr, LOG_ERROR,
               "avr_loadcode(): Attempted to load code of size %d but flash size is only %d.\n",
               size, avr->flashend + 1);
      abort ();
    }
...
avr_t *
avr_core_allocate (const avr_t * core, uint32_t coreLen)
{
...
avr_t *
avr_make_mcu_by_name (const char *name)
{
