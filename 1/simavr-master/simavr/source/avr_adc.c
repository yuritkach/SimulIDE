...
avr_adc_int_raise (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
...
static uint8_t
avr_adc_read_l (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_adc_t *p = (avr_adc_t *) param;
 ...
 * So here if the H is read before the L, we still call the L to update the register value.
 */
static uint8_t
avr_adc_read_h (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_adc_t *p = (avr_adc_t *) param;
...
static void
avr_adc_configure_trigger (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_adc_t *p = (avr_adc_t *) param;
  ...
  if (!adsc && avr_regbit_get (avr, p->adsc))
    {
      ...
      avr_cycle_timer_register (avr, avr_hz_to_cycles (avr, div), avr_adc_int_raise, p);
    }
  avr_core_watch_write (avr, addr, v);
}

static void
avr_adc_write_adcsrb (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_adc_configure_trigger (avr, addr, v, param);
}

static void
avr_adc_irq_notify (struct avr_irq_t *irq, uint32_t value, void *param)
{
  avr_adc_t *p = (avr_adc_t *) param;
  avr_t *avr = p->io.avr;
...
void
avr_adc_init (avr_t * avr, avr_adc_t * p)
{
  p->io = _io;
