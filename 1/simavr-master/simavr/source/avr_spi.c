...
avr_spi_raise (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
...
static uint8_t
avr_spi_read (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_spi_t *p = (avr_spi_t *) param;
...
static void
avr_spi_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_spi_t *p = (avr_spi_t *) param;
...
static void
avr_spi_irq_input (struct avr_irq_t *irq, uint32_t value, void *param)
{
  avr_spi_t *p = (avr_spi_t *) param;
  avr_t *avr = p->io.avr;
...
void
avr_spi_reset (struct avr_io_t *io)
{
...
void
avr_spi_init (avr_t * avr, avr_spi_t * p)
{
  p->io = _io;
