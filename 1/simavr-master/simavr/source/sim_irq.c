...
static void
_avr_irq_pool_remove (avr_irq_pool_t * pool, avr_irq_t * irq)
{
...
_avr_alloc_irq_hook (avr_irq_t * irq)
{
...
void
avr_free_irq (avr_irq_t * irq, uint32_t count)
{
  if (!irq || !count)
    return;
  for (int i = 0; i < count; i++)
...
void
avr_irq_register_notify (avr_irq_t * irq, avr_irq_notify_t notify, void *param)
{
  if (!irq || !notify)
    return;
...
void
avr_irq_unregister_notify (avr_irq_t * irq, avr_irq_notify_t notify, void *param)
{
...
void
avr_unconnect_irq (avr_irq_t * src, avr_irq_t * dst)
{
  avr_irq_hook_t *hook, *prev;
