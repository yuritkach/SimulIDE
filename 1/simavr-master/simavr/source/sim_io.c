...
int
avr_ioctl (avr_t * avr, uint32_t ctl, void *io_param)
{
...
void
avr_register_io (avr_t * avr, avr_io_t * io)
{
  io->next = avr->io_port;
  io->avr = avr;
  avr->io_port = io;
...
void
avr_register_io_read (avr_t * avr, avr_io_addr_t addr, avr_io_read_t readp, void *param)
{
...
void
avr_register_io_write (avr_t * avr, avr_io_addr_t addr, avr_io_write_t writep, void *param)
{
  avr_io_addr_t a = AVR_DATA_TO_IO (addr);
  ...
  if (a >= MAX_IOs)
    {
      AVR_LOG (avr, LOG_ERROR,
               "IO: avr_register_io_write(): IO address 0x%04x out of range (max 0x%04x).\n", a,
               MAX_IOs);
      abort ();
    }
  /*
   * Verifying that some other piece of code is not installed to watch write on this address. If
   * there is, this code installs a "dispatcher" callback instead to handle multiple clients,
          ...
          if (avr->io[a].w.c != _avr_io_mux_write)
            {
              ...
              if (avr->io_shared_io_count > 4)
                {
                  AVR_LOG (avr, LOG_ERROR,
                           "IO: avr_register_io_write(): Too many shared IO registers.\n");
                  abort ();
                }
              AVR_LOG (avr, LOG_TRACE,
                       "IO: avr_register_io_write(%04x): Installing muxer on register.\n", addr);
...
avr_irq_t *
avr_iomem_getirq (avr_t * avr, avr_io_addr_t addr, const char *name, int index)
{
  if (index > 8)
    return NULL;
  avr_io_addr_t a = AVR_DATA_TO_IO (addr);
  if (avr->io[a].irq == NULL)
    {
      /*
       * Prepare an array of names for the io IRQs. Ideally we'd love to have a proper name for
       * these, but it's not possible at this time.
       */
      char names[9 * 20];
      char *d = names;
      const char *namep[9];
...
avr_irq_t *
avr_io_setirqs (avr_io_t * io, uint32_t ctl, int count, avr_irq_t * irqs)
{
  ...
  if (!irqs)
    {
      ...
      irqs = avr_alloc_irq (&io->avr->irq_pool, 0, count, irq_names);
      if (irq_names)
        {
          for (int i = 0; i < count; i++)
