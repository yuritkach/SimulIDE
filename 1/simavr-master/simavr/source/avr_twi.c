...
/*
 * This block respectfully nicked straight out from the Atmel sample code for AVR315. Typos and all.
...
static inline void
_avr_twi_status_set (avr_twi_t * p, uint8_t v, int interrupt)
{
...
inline uint8_t _avr_twi_status_get (avr_twi_t * p)
{
  return avr_regbit_get_raw (p->io.avr, p->twsr);
}

static avr_cycle_count_t
avr_twi_set_state_timer (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
...
/*
 * This is supposed to trigger a timer whose duration is a multiple of 'twi' clock cycles, which
 * should be derived from the prescaler (100khz, 400khz etc).
 * Right now it cheats and uses one twi cycle == one usec.
 */
static void
_avr_twi_delay_state (avr_twi_t * p, int twi_cycles, uint8_t state)
{
...
static void
avr_twi_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_twi_t *p = (avr_twi_t *) param;
  ...
  if (p->state & TWI_COND_SLAVE)
      ...
      if (p->state & TWI_COND_ADDR)
        {
          ...
          if (do_read)
            {
              if (p->state & TWI_COND_WRITE)
                {
                  avr_raise_irq (p->io.irq + TWI_IRQ_OUTPUT,
                                 avr_twi_irq_msg (TWI_COND_READ | TWI_COND_ACK, p->peer_addr,
                                                  avr->data[p->r_twdr]));
                }
#if AVR_TWI_DEBUG
              else
          ...
          avr_raise_irq (p->io.irq + TWI_IRQ_OUTPUT,
                         avr_twi_irq_msg (TWI_COND_ADDR + (do_ack ? TWI_COND_ACK : 0), p->peer_addr,
                                          avr->data[p->r_twdr]));
        }
      else
  ...
  else
    {
	  ...
	  // if the latch is ready... as set by writing/reading the TWDR
          if (p->state & msgv)
            {
	      // we send an IRQ and we /expect/ a slave to reply immediately via an IRQ to set the
	      // COND_ACK bit otherwise it's assumed it's been nacked...
              avr_raise_irq (p->io.irq + TWI_IRQ_OUTPUT, avr_twi_irq_msg (msgv, p->peer_addr, avr->data[p->r_twdr]));
	  ...
	  // we send an IRQ and we /expect/ a slave to reply immediately via an IRQ tp set the
	  // COND_ACK bit otherwise it's assumed it's been nacked...
          avr_raise_irq (p->io.irq + TWI_IRQ_OUTPUT, avr_twi_irq_msg (TWI_COND_START, p->peer_addr, 0));
 ...
 */
static void
avr_twi_write_data (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_twi_t *p = (avr_twi_t *) param;
 ...
 */
static uint8_t
avr_twi_read_data (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_twi_t *p = (avr_twi_t *) param;
 ...
 */
static void
avr_twi_write_status (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_twi_t *p = (avr_twi_t *) param;
...
static void
avr_twi_irq_input (struct avr_irq_t *irq, uint32_t value, void *param)
{
  avr_twi_t *p = (avr_twi_t *) param;
  avr_t *avr = p->io.avr;
...
uint32_t
avr_twi_irq_msg (uint8_t msg, uint8_t addr, uint8_t data)
{
