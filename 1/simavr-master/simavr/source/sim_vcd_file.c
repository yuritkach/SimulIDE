...
int
avr_vcd_init (struct avr_t *avr, const char *filename, avr_vcd_t * vcd, uint32_t period)
{
  ...
  // No longer initializes the IRQs here, they can be initialized on the fly when traces are added

  return 0;
}

void
avr_vcd_close (avr_vcd_t * vcd)
{
  avr_vcd_stop (vcd);
}

void
_avr_vcd_notify (struct avr_irq_t *irq, uint32_t value, void *param)
{
...
static char *
_avr_vcd_get_float_signal_text (avr_vcd_signal_t * s, char *out)
{
  char *dst = out;
...
static char *
_avr_vcd_get_signal_text (avr_vcd_signal_t * s, char *out, uint32_t value)
{
  char *dst = out;
...
static void
avr_vcd_flush_log (avr_vcd_t * vcd)
{
  ...
  for (uint32_t li = 0; li < vcd->logindex; li++)
    {
      ...
      // if that trace was seen in this nsec already, we fudge the base time to make sure the new
      // value is offset by one nsec, to make sure we get at least a small pulse on the waveform
      // This is a bit of a fudge, but it is the only way to represent very short"pulses" that are
      // still visible on the waveform.
...
_avr_vcd_timer (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
  avr_vcd_t *vcd = param;
...
int
avr_vcd_add_signal (avr_vcd_t * vcd, avr_irq_t * signal_irq, int signal_bit_size, const char *name)
{
