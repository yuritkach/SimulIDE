...
static void
avr_watchdog_run_callback_software_reset (avr_t * avr)
{
  avr_reset (avr);
}

static avr_cycle_count_t
avr_watchdog_timer (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
  ...
  if (avr_regbit_get (avr, p->watchdog.enable))
  ...
  else if (avr_regbit_get (avr, p->wde))
    {
       ...
       * It seems our best (and cleanest) solution is to set a temporary call back which can safely
       * perform the reset for us...  During reset, the previous callback can be restored and safely
       * resume.
       */
...
static avr_cycle_count_t
avr_wdce_clear (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
  avr_watchdog_t *p = (avr_watchdog_t *) param;
...
void
avr_watchdog_init (avr_t * avr, avr_watchdog_t * p)
{
  p->io = _io;
