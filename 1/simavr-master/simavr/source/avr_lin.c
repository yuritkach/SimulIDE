...
static void
avr_lin_baud_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_lin_t *p = (avr_lin_t *) param;

  if (p->r_linbtr != p->ldisr.reg || p->r_linbtr != p->lbt.reg)
    {   // sanity check
...
void
avr_lin_init (avr_t * avr, avr_lin_t * p)
{
  // init uart part
