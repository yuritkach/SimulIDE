...
avr_register_vector (avr_t * avr, avr_int_vector_t * vector)
{
  ...
  if (vector->trace)
    printf ("%s register vector %d (enabled %04x:%d)\n", __FUNCTION__, vector->vector,
            vector->enable.reg, vector->enable.bit);
...
int
avr_has_pending_interrupts (avr_t * avr)
{
  avr_int_table_p table = &avr->interrupts;
...
int
avr_is_interrupt_pending (avr_t * avr, avr_int_vector_t * vector)
{
  return vector->pending;
}

int
avr_is_interrupt_enabled (avr_t * avr, avr_int_vector_t * vector)
{
  return avr_regbit_get (avr, vector->enable);
}

int
avr_raise_interrupt (avr_t * avr, avr_int_vector_t * vector)
{
  ...
  // always mark the 'raised' flag to one, even if the interrupt is disabled this allow "polling"
  // for the "raised" flag, like for non-interrupt driven UART and so so. These flags are often
  // "write one to clear"
...
void
avr_clear_interrupt (avr_t * avr, avr_int_vector_t * vector)
{
  if (!vector)
    return;
  if (vector->trace)
...
int
avr_clear_interrupt_if (avr_t * avr, avr_int_vector_t * vector, uint8_t old)
{
...
avr_irq_t *
avr_get_interrupt_irq (avr_t * avr, uint8_t v)
{
  avr_int_table_p table = &avr->interrupts;
  for (int i = 0; i < table->vector_count; i++)
...
/*
 * check whether interrupts are pending. If so, check if the interrupt "latency" is reached, and if
 * so triggers the handlers and jump to the vector.
 */
void
avr_service_interrupts (avr_t * avr)
{
  if (!avr->sreg[S_I])
    return;
  ...
  int cnt = table->pending_w > table->pending_r ?
    table->pending_w - table->pending_r : (table->pending_w + INT_FIFO_SIZE) - table->pending_r;
  ...
  // if that single interrupt is masked, ignore it and continue could also have been disabled, or
  // cleared
