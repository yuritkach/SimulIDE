...
avr_uart_txc_raise (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
...
static avr_cycle_count_t
avr_uart_rxc_raise (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
  avr_uart_t *p = (avr_uart_t *) param;
...
static uint8_t
avr_uart_rxc_read (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_uart_t *p = (avr_uart_t *) param;
  ...
  //
  // if RX is enabled, and there is nothing to read, and the AVR core is reading this register, it's
  // probably to poll the RXC TXC flag and spinloop so here we introduce a usleep to make it a bit
  // lighter on CPU and let data arrive
  //
...
static uint8_t
avr_uart_read (struct avr_t *avr, avr_io_addr_t addr, void *param)
{
  avr_uart_t *p = (avr_uart_t *) param;
...
static void
avr_uart_baud_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_uart_t *p = (avr_uart_t *) param;
...
static void
avr_uart_udr_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_uart_t *p = (avr_uart_t *) param;
...
static void
avr_uart_write (struct avr_t *avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_uart_t *p = (avr_uart_t *) param;
...
static void
avr_uart_irq_input (struct avr_irq_t *irq, uint32_t value, void *param)
{
  avr_uart_t *p = (avr_uart_t *) param;
  avr_t *avr = p->io.avr;
  ...
  TRACE (printf
         ("UART IRQ in %02x (%d/%d) %s\n", value, p->input.read, p->input.write,
          uart_fifo_isfull (&p->input) ? "FULL!!" : "");) if (uart_fifo_isfull (&p->input))
    avr_raise_irq (p->io.irq + UART_IRQ_OUT_XOFF, 1);
}

void
avr_uart_reset (struct avr_io_t *io)
{
...
static int
avr_uart_ioctl (struct avr_io_t *port, uint32_t ctl, void *io_param)
{
...
void
avr_uart_init (avr_t * avr, avr_uart_t * p)
{
  p->io = _io;
