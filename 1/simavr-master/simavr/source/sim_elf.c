...
void
avr_load_firmware (avr_t * avr, elf_firmware_t * firmware)
{
  ...
  for (int i = 0; i < firmware->symbolcount; i++)
    if (firmware->symbol[i]->addr < firmware->flashsize)   // code address
      avr->trace_data->codeline[firmware->symbol[i]->addr >> 1] = firmware->symbol[i];
  ...
  avr_vcd_init (avr,
                ...
                avr->vcd, firmware->traceperiod >= 1000 ? firmware->traceperiod : 1000);
...
static void
elf_parse_mmcu_section (elf_firmware_t * firmware, uint8_t * src, uint32_t size)
{
  while (size)
    {
      uint8_t tag = *src++;
      ...
      switch (tag)
        ...
        case AVR_MMCU_TAG_VCC:
          firmware->vcc = src[0] | (src[1] << 8) | (src[2] << 16) | (src[3] << 24);
          break;
        case AVR_MMCU_TAG_AVCC:
          firmware->avcc = src[0] | (src[1] << 8) | (src[2] << 16) | (src[3] << 24);
          break;
        case AVR_MMCU_TAG_AREF:
          firmware->aref = src[0] | (src[1] << 8) | (src[2] << 16) | (src[3] << 24);
          break;
              ...
              if (!firmware->external_state[i].port)
                {
                  ...
                  AVR_LOG (NULL, LOG_TRACE, "AVR_MMCU_TAG_PORT_EXTERNAL_PULL[%d] %c:%02x:%02x\n",
                           ...
                           firmware->external_state[i].mask, firmware->external_state[i].value);
                  break;
                }
          }
          break;
  ...
  while ((scn = elf_nextscn (elf, scn)) != NULL)
    {
	  ...
	  // how many symbols are there? this number comes from the size of the section divided by
	  // the entry size
              ...
              if (ELF32_ST_BIND (sym.st_info) == STB_GLOBAL ||
                  ...
                  ELF32_ST_TYPE (sym.st_info) == STT_OBJECT)
                {
                  ...
                  if (!(firmware->symbolcount % 8))
                    firmware->symbol = realloc (firmware->symbol,
                                                (firmware->symbolcount +
                                                 8) * sizeof (firmware->symbol[0]));
