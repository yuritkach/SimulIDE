...
avr_eempe_clear (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
...
static avr_cycle_count_t
avr_eei_raise (struct avr_t *avr, avr_cycle_count_t when, void *param)
{
  avr_eeprom_t *p = (avr_eeprom_t *) param;
...
static void
avr_eeprom_write (avr_t * avr, avr_io_addr_t addr, uint8_t v, void *param)
{
  avr_eeprom_t *p = (avr_eeprom_t *) param;
...
static int
avr_eeprom_ioctl (struct avr_io_t *port, uint32_t ctl, void *io_param)
{
...
static void
avr_eeprom_dealloc (struct avr_io_t *port)
{
  avr_eeprom_t *p = (avr_eeprom_t *) port;
  if (p->eeprom)
    free (p->eeprom);
...
void
avr_eeprom_init (avr_t * avr, avr_eeprom_t * p)
{
  p->io = _io;
