...
static void
gdb_watch_clear (avr_gdb_watchpoints_t * w)
{
  w->len = 0;
}

static void
gdb_send_reply (avr_gdb_t * g, char *cmd)
{
  uint8_t reply[1024];
  uint8_t *dst = reply;
  uint8_t check = 0;
  *dst++ = '$';
  ...
  DBG (printf ("%s '%s'\n", __FUNCTION__, reply);) send (g->s, reply, dst - reply + 3, 0);
}

static void
gdb_send_quick_status (avr_gdb_t * g, uint8_t signal)
{
  char cmd[64];
...
static int
gdb_change_breakpoint (avr_gdb_watchpoints_t * w,
                       int set, enum avr_gdb_watch_type kind, uint32_t addr, uint32_t size)
{
...
static int
gdb_write_register (avr_gdb_t * g, int regi, uint8_t * src)
{
  switch (regi)
    {
    case 0 ... 31:
...
static int
gdb_read_register (avr_gdb_t * g, int regi, char *rep)
{
  switch (regi)
    {
    case 0 ... 31:
...
static void
gdb_handle_command (avr_gdb_t * g, char *cmd)
{
  avr_t *avr = g->avr;
  char rep[1024];
  uint8_t command = *cmd++;
  switch (command)
      ...
      if (strncmp (cmd, "Supported", 9) == 0)
      ...
      else if (strncmp (cmd, "Xfer:memory-map:read", 20) == 0)
        {
          snprintf (rep, sizeof (rep),
                    ...
                    " </memory>\n" "</memory-map>", g->avr->ramend + 1, g->avr->flashend + 1);
    ...
    case 's':
      ...
      break;
    case 'r':
      {   // deprecated, suggested for AVRStudio compatibility
...
static int
gdb_network_handler (avr_gdb_t * g, uint32_t dosleep)
{
  fd_set read_set;
  int max;
  FD_ZERO (&read_set);
  ...
  if (kind & type)
    {
      ...
      sprintf (cmd, "T%02x20:%02x;21:%02x%02x;22:%02x%02x%02x00;%s:%06x;",
               ...
               kind & AVR_GDB_WATCH_WRITE ? "watch" : "rwatch", addr | 0x800000);
      gdb_send_reply (g, cmd);
...
int
avr_gdb_processor (avr_t * avr, int sleep)
{
  if (!avr || !avr->gdb)
    return 0;
  avr_gdb_t *g = avr->gdb;
