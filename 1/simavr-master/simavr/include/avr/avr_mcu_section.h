...
#define AVR_MCU_BYTE(_tag, _val)                        \
  ...
  /*!
   * This Macro allows you to specify traces for the VCD file output engine. This specifies a
   * default header, and let you fill in the relevant bits.
   * Example:
...
#define AVR_MCU_VCD_FILE(_name, _period)                \
  ...
  /*!
   * It is possible to send "commands" to simavr from the firmware itself. For this to work you need
   * to specify an IO register that is to be used for a write-only bridge. A favourite is one of the
...
#define AVR_MCU_SIMAVR_COMMAND(_register)                               \
  ...
  /*!
   * Similar to AVR_MCU_SIMAVR_COMMAND, The CONSOLE allows the AVR code to declare a register
...
#define AVR_MCU_SIMAVR_CONSOLE(_register)                               \
  ...
  /*!
   * Allows the firmware to hint simavr as to wether there are external pullups/down on PORT
   * pins. It helps if the firmware uses "open drain" pins by toggling the DDR pins to switch
...
#define AVR_MCU_EXTERNAL_PORT_PULL(_port, _mask, _val)  \
  ...
  /*!
   * This tag allows you to specify the voltages used by your board It is optional in most cases,
   * but you will need it if you use ADC module's IRQs. Not specifying it in this case might lead to
