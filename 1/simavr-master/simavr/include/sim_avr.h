...
#define STACK_FRAME_SIZE 32
    /// this records the call/ret pairs, to try to catch code that munches the stack -under- their
    /// own frame
    struct
    {
      uint32_t pc;
  ...
  typedef struct avr_t
  {
    ...
    uint32_t codeend;   ///< filled by the ELF data, this allow tracking of invalid jumps
    ...
    uint8_t trace:1, log:2;   ///< log level, default to 1
    ...
    /// if non-zero, the gdb server will be started when the core crashed even if not activated at
    /// startup if zero, the simulator will just exit() in case of a crash
    int gdb_port;
  } avr_t;
  ...
  /**
   * These are accessors for avr->data but allows watchpoints to be set for gdb IO modules use that
   * to set values to registers, and the AVR core decoder uses that to register "public" read by
   * instructions.
   */
