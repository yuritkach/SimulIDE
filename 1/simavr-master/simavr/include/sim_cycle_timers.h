 ...
 *
 * The implementation maintains a list of 'pending' timers, sorted by when they should run, it
 * allows very quick comparison with the next timer to run, and quick removal of then from the pile
 * once dispatched.
 */
...
#define MAX_CYCLE_TIMERS 64
  ...
  /**
   * Each timer instance contains the absolute cycle number they are hoping to run at, a function
   * pointer to call and a parameter
   * 
   * it will NEVER be the exact cycle specified, as each instruction is not divisible and might take
   * 2 or more cycles anyway.
   * 
   * However if there was a LOT of cycle lag, the timer migth be called repeteadly until it 'caches
   * up'.
   */
  ...
  /**
   * Timer pool contains a pool of timer slots available, they all start queued into the 'free'
   * qeueue, are migrated to the 'active' queue when needed and are re-queued to the free one when
   * done
   */
  ...
  /**
   * Check to see if a timer is present, if so, return the number (+1) of cycles left for it to
   * fire, and if not present, return zero
   */
